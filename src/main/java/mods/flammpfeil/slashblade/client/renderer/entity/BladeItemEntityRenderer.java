            ...
            try (MSAutoCloser msac = MSAutoCloser.pushMatrix(matrixStackIn)) {
                ...
                try (MSAutoCloser msac2 = MSAutoCloser.pushMatrix(matrixStackIn)) {
                    ...
                    if (itemIn.isInWater()) {
                        ...
                        matrixStackIn.mulPose(new Quaternion(0, (float) Math.toDegrees(itemIn.bobOffs), 0, true));
                        ...
                        float speed = -81f;
                        matrixStackIn.mulPose(new Quaternion(0, 0, speed * (itemIn.tickCount + partialTicks), true));
                        matrixStackIn.translate(xOffset, 0, 0);
                    } else {
                        matrixStackIn.scale(scale, scale, scale);

                        matrixStackIn
                                .mulPose(new Quaternion(0, 0, 60 + (float) Math.toDegrees(itemIn.bobOffs / 6.0), true));
